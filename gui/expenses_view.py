...
def refresh_expense(logic):
    ...
    def create_expense_card(parent, row, col, expense_id, amount, category, desc, date):
        ...
        # remove button
        t_button = tk.Button(top_row, text="Remove", font=("Calibri", 10), fg=COLOR_DANGER, bg=COLOR_CARD_BG, bd=0, cursor="hand2", command=lambda: handle_card_action("remove", expense_id))
        t_button.pack(side="right")

        # Edit button
        e_button = tk.Button(top_row, text="Edit", font=("Calibri", 10), fg=COLOR_EDIT, bg=COLOR_CARD_BG, bd=0, cursor="hand2", command=lambda: handle_card_action("edit", expense_id))
